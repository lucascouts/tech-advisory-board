...
## 7. Protect TAB directories (optional)

Plugins cannot declare `additionalDirectories` in their own
`settings.json` — the key is silently ignored by the loader (the
official docs list only `agent` and `subagentStatusLine` as valid in
plugin settings). To block accidental edits to `.tab/sessions/` and
`.tab/decisions/`, add this to `~/.claude/settings.json` (user scope) OR
to a drop-in under `managed-settings.d/`:

```json
// 50-tab-protect-dirs.json
{
  "additionalDirectories": [".tab/sessions", ".tab/decisions"]
}
```

Effect: Claude Code requires confirmation before any Write/Edit to the
listed paths. Requires Claude Code ≥ v2.1.90.
