...
The Technical Advisory Board plugin uses three recommended MCP servers for **current, verifiable web research**. They are not bundled with the plugin — Claude Code plugin validators do not allow plugins to declare `mcpServers` directly. You install them once in the host session and every TAB run picks them up automatically.

## Why MCPs matter

| Without MCPs | With MCPs |
|---|---|
| `WebSearch` fallback only | `perplexity_search` + `perplexity_research` + `context7` + `brave-search` |
...
| Auditor `spot-check` phase is best-effort | Auditor runs Perplexity `reason` + Context7 `query` per high-stake claim |
...
This script inspects `~/.claude/settings.json` and `$CLAUDE_PROJECT_DIR/.claude/settings.json` and prints which recommended MCPs are configured, missing, or misconfigured. It makes no network calls and never mutates state.
