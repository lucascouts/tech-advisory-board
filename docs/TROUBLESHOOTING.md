...
- `skills/tab/SKILL.md` runs `init-dir`, `load-config.sh`,
  `extract-context.sh`, `check-mcps` via shell blocks.
- `skills/rechallenge/SKILL.md` runs `init-dir`,
  `supersede-adr`, `new-adr` via shell blocks.
...
close the session (`resume-session --abort`) so compaction can run.
...
rechallenge from inside a worktree, re-run `schedule-rechallenge`
from the main repo.


## `channel` push is not sending messages

### Likely cause: `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=1`

When `CLAUDE_CODE_SUBPROCESS_ENV_SCRUB=1` is active, subprocesses
(including MCP servers shipped by plugins) lose access to environment
variables such as `TELEGRAM_BOT_TOKEN`, `TELEGRAM_CHAT_ID`, and
`CHANNEL_WEBHOOK_URL`. The bundled `servers/channel/index.js`
server depends on those vars and degrades silently — `sendMessage`
returns `degraded: true, ok: false` and emits a single stderr line
(`[channel] degraded mode: no transport env vars present`).

The `Notification` hook (`scripts/on-notification.sh`, item 04) routes
failed Rechallenge sessions through this same MCP, so under ENV_SCRUB
the push itself does not fire either. The archived session under
`.tab/sessions/<id>/` is still the source of truth — failure is never
lost, only the push notification is suppressed.

**Immediate fix** (acceptable): unset the scrub for the session, or
export the transport env vars explicitly before launching Claude Code.
...
**Definitive fix** (roadmap): migrate `channel` to read credentials
from `${CLAUDE_PLUGIN_DATA}/channel/credentials.json` when env
vars are absent, so push works under ENV_SCRUB without weakening host
hardening. Tracked as a follow-up in the project issue tracker.

**Diagnose:**

```bash
node servers/channel/index.js --test 2>&1 | tail -1
# degraded:true → env vars not visible to the subprocess
```
