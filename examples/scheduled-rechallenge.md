...
```bash
schedule-rechallenge .tab/decisions/0003-database-selection.md \
    --every "0 9 * * 1" \
    --mode quick
```
...
- `synthesis.json` — with `session.mode = "Rechallenge"` and a `verdict` field:
  ...
  - `supersede` — an alternative now dominates; `new-adr` writes a new ADR and `supersede-adr` marks the old one.
