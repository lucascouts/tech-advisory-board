...
Launch ONE `tab-researcher` subagent per shortlisted alternative:
...
Champions are launched as **parallel `tab-champion` subagents** (Opus, max effort).
...
Launch N `tab-champion` agents in parallel (cross-exam mode):
...
Advisors are launched as **parallel `tab-advisor` subagents** (Sonnet, max effort).
...
1. Run `bash ${CLAUDE_SKILL_DIR}/scripts/validate-synthesis.sh` mentally
