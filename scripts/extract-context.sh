...
if [ -f "$TARGET" ]; then
    ...
    MANIFEST=""
    while [ "$dir" != "/" ]; do
        for f in package.json requirements.txt pyproject.toml Cargo.toml go.mod pom.xml build.gradle; do
            if [ -f "$dir/$f" ]; then
                PROJECT_ROOT="$dir"
                MANIFEST="$f"
                break 2
...
for f in package.json requirements.txt pyproject.toml Cargo.toml go.mod \
         pom.xml build.gradle build.sbt mix.exs Gemfile composer.json; do
    [ -f "$f" ] && MANIFESTS+=("$f")
done
...
target = '$TARGET'
manifests = '${MANIFESTS[*]:-}'.split() if '${MANIFESTS[*]:-}' else []
configs = '${CONFIGS[*]:-}'.split() if '${CONFIGS[*]:-}' else []
