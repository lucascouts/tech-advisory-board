...
SHOULD_INJECT=$(python3 - "$INPUT" "${CLAUDE_PLUGIN_OPTION_strict_paths:-false}" "${PWD}" <<'PYEOF' 2>/dev/null || echo "no"
...
strict_paths = sys.argv[2].lower() in ("true", "1", "yes")
cwd = sys.argv[3]

prompt = (payload.get("user_prompt") or payload.get("prompt") or "").lower()

# Manual invocation via /tech-advisory-board: always wins, regardless
# of strict_paths — users that explicitly type the skill mean it.
manual_invocation = "/tech-advisory-board:" in prompt
...
matched = any(re.search(pat, prompt) for pat in triggers)
if not matched:
    print("no"); sys.exit(0)

# strict_paths gate: only inject when the cwd carries a project manifest.
# Manual invocation bypasses the gate so greenfield prompts still work
# when the user types /tech-advisory-board:tab explicitly.
if strict_paths and not manual_invocation:
    manifests = (
        "package.json", "pyproject.toml", "Cargo.toml", "go.mod",
        "pom.xml", "Gemfile", "composer.json", "build.gradle",
        "build.gradle.kts", "setup.py", "requirements.txt",
    )
    if not any(os.path.exists(os.path.join(cwd, m)) for m in manifests):
        print("no"); sys.exit(0)
