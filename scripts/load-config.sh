...
#   CLAUDE_PLUGIN_OPTION_default_mode             → default_mode
set -uo pipefail
...
# default_mode (userConfig) — canonical names only
# (Instant, Fast, Standard, Complete, Complete+).
v = getenv_str("default_mode")
if v is not None:
    merged["default_mode"] = v
