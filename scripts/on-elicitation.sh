...
#
# Concurrency: append goes through scripts/lib/json_atomic.py so
# simultaneous Elicitation + ElicitationResult firings don't race.
set -uo pipefail
...
tel_path = Path(session_dir) / "telemetry.json"
...
def append_elicitation(tel):
    tel.setdefault("elicitations", []).append(record)
    return tel

try:
    atomic_update(tel_path, append_elicitation, default={}, timeout_seconds=1.5)
