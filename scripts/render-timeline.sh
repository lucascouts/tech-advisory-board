...
(async () => {
  ...
  async function fetchTail(offset) {
    try {
      ...
      // Rotation detection: scripts/monitor.sh rotates the NDJSON when
      // it reaches its cap (default 10k lines / 5MB, see M2 in
      // ANALISE-CRITICA.md). The old tail moves to <file>.1 and the
      // live file starts empty. If the current length is below our
      // offset, rotation happened and we should re-read from 0.
      const effectiveOffset = lines.length < offset ? 0 : offset;
      const events = lines.slice(effectiveOffset)
  ...
  let offset = 0;
  async function tick() {
    const res = await fetchTail(offset);
    if (res === null) {
      $("status").innerHTML = `<span class="dot warn"></span>NDJSON not readable — open this HTML from the session directory`;
      return;
    }
    // Track the absolute position in the live file (not cumulative)
    // so rotation resets cleanly.
    offset = res.total;
    if (res.events.length) {
      for (const ev of res.events) applyEvent(ev);
      render();
    }
  }
  tick();
