...
- Link the supersession:
  ```bash
  !`${CLAUDE_PLUGIN_ROOT}/bin/tab-supersede-adr --old <original-ADR> --new <new-ADR>`
  ```
...
- Do NOT silently mutate the original ADR. Every change to the original
  ...
  status-line change via `tab-supersede-adr`.
...
3. **Recursion safety.** Rechallenging an ADR that was itself produced
   ...
   `tab-supersede-adr` handles N-deep chains.
4. **Checkpointing does NOT track Bash mutations.** `tab-supersede-adr`
   rewrites the original ADR's status line and appends a "Superseded By"
   section via shell, and `tab-new-adr` creates new files via Bash.
   ...
   snapshot to the user before invoking `tab-supersede-adr`.
