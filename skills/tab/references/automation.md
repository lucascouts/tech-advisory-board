...
| `TAB_RATE_OPUS_IN` / `TAB_RATE_OPUS_OUT` / etc. | Override `compute-cost` per-Mtok rates |
...
blanket bypass, **never** ship this to a shared runner), use a scoped
`PreToolUse` hook that returns `permissionDecision: "defer"` **only** for
the two write paths TAB uses in CI:

- `Bash(${CLAUDE_PLUGIN_ROOT}/bin/new-adr ...)`
- `Bash(${CLAUDE_PLUGIN_ROOT}/bin/supersede-adr ...)`

`"defer"` (Claude Code ≥ v2.1.89) means *"pause here; the host will resume
once an external approval arrives"*. The headless session parks; you flip
it back on with `claude -p --resume <session_id>` after the reviewer
signs off. Everything else (arbitrary `Bash`, `Edit`, `Write`) stays
gated.

### Full recipe — hook + approval + resume

```yaml
- name: Run TAB with ADR-write deferral
  id: run_tab
  env:
    ENABLE_PROMPT_CACHING_1H: "1"
    CLAUDE_CODE_OAUTH_TOKEN: ${{ secrets.CLAUDE_CODE_OAUTH_TOKEN }}
  run: |
    PLUGIN_ROOT=$(claude plugin path tech-advisory-board)
    ...
    claude -p "/tech-advisory-board:tab \"$QUESTION\"" \
      --bare --output-format stream-json --include-partial-messages \
...
- name: Approve and resume
  ...
  env:
    CLAUDE_CODE_OAUTH_TOKEN: ${{ secrets.CLAUDE_CODE_OAUTH_TOKEN }}
  run: claude -p --resume "${{ steps.run_tab.outputs.session_id }}" --bare
```

Auditable, scoped, and reversible. Keep the defer matcher tight to
`${CLAUDE_PLUGIN_ROOT}/bin/new-adr` and `supersede-adr` — broader
matchers defeat the purpose. See `examples/ci-github-actions.yml` for a
...
## 7.4 Output styles — `terse` and `presentation`

TAB ships two `outputStyles` for tailoring the final synthesis to its
audience:

- **`terse`** — ≤20 lines, no markdown, no prose. Intended for CI logs,
  stream-json consumers, and anywhere a human-readable report would be
  noise. Emits `mode · verdict · primary.stack · confidence · top-3
  risks · adr_path`.
- **`presentation`** — structured headings (Decisão / Riscos / Evolução
  / Próximos passos), accessible language, ≤400 words. Intended for
  stakeholder review where technical jargon is a barrier.

Invoke via the standard `--output-style` flag:

```bash
claude -p --output-style terse \
  "/tech-advisory-board:tab \"stack for order-service\""

claude -p --output-style presentation \
  "/tech-advisory-board:tab \"migrar monolito para microserviços?\""
```

Output-style files live in `output-styles/` at the plugin root. The
styles do not alter the `synthesis.json` contract — they affect only the
human-readable assistant message rendered alongside it.
