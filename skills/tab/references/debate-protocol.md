...
2. **Normalize via z-score (per-advisor)** — pipe the matrix into
   `${CLAUDE_PLUGIN_ROOT}/bin/score-zscore` to compute `score_z` per
...
1. Walk through the required-sections checklist from
   `references/synthesis-template.md` before emitting the draft
   (mode-specific: Instant skips champions/advisors, Standard+ requires
   them). The Stop hook will run `validate-synthesis-json.sh` against
   the emitted JSON; use the template as a pre-flight check so the
   Moderator does not ship a draft that fails the gate.
