...
- Validation: `validate-synthesis-json.sh` still runs at the Stop gate,
  but many assertions are mode-skipped for Instant (no champions,
  no advisors, no auditor). Manual review of the Moderator's direct
...
User presents question
       |
       v
BOOTSTRAP [Phase -1]  (init-dir + load-config + resume-session)
