...
enforced alongside the query budget via `compute-cost`.
...
```bash
cache list                       # show all shared entries with freshness
cache list --category volatile   # filter
cache stats                      # counts by freshness + category
cache invalidate "nextjs"        # manual eviction for a topic substring
cache prune                      # drop expired / invalidated entries
cache path                       # resolved cache file path
```

The per-session `research-cache.json` (scope `"session"`) is not managed by
`cache`; it is scoped to the session dir and discarded on archival.
