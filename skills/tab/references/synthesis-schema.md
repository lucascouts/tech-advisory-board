...
| `conflicts_of_interest` | Complete/Complete+/Rechallenge | Non-empty in adversarial modes; optional in Instant/Fast/Standard |
...
### 2.14 `conflicts_of_interest`

**Hard-required and non-empty** in `Complete`, `Complete+`, `Rechallenge`.
Each entry is a COI card emitted by subagents that carry `memory: project`
(currently `researcher` and `auditor`). **Optional** in `Instant`, `Fast`,
`Standard` — present only if the Moderator explicitly invoked an agent
with project memory.

Schema per entry (see `schemas/synthesis.schema.json` for the authoritative
definition):

- `agent` *(string)* — the subagent's name / alias for this session.
- `agent_type` *(enum: `"researcher" | "auditor"`)* — which role the card
  reports from.
- `memory_loaded` *(integer)* — how many entries were loaded from the
  scoped `MEMORY.md` for this invocation.
- `bias_signal` *(enum: `"none" | "low" | "medium" | "high"`)* — the
  agent's self-assessed bias risk toward previously-seen stacks.
- `mitigations` *(array of strings)* — mitigations applied (identity
  rotation, memory scoping, adversarial role toward prior pick, etc.).

**Stop gate**: `scripts/validate-synthesis-json.sh` rejects sessions in
adversarial modes (`Complete` / `Complete+` / `Rechallenge`) with exit 1
when `conflicts_of_interest[]` is absent or malformed. In
`Instant` / `Fast` / `Standard`, the array is optional and validator
emits a warning-level entry only if the shape is malformed.

Full protocol and card format: `references/coi-disclosure.md`.
